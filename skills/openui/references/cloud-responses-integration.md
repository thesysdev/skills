...
Read [cloud-integration.md](cloud-integration.md) first. Use this runbook to add OpenUI Cloud to an existing application using Responses events and, when requested, Cloud-managed conversations, hosted tools, or artifacts in the agent stream. Do not apply it to an existing Chat Completions application unless the user has chosen a protocol migration.

## Contents

1. [Supported Contract](#supported-contract)
2. [Choose the Responses State Model](#choose-the-responses-state-model)
3. [Audit the Host](#audit-the-host)
4. [Install and Configure](#install-and-configure)
...
The detailed Cloud Conversations path in this runbook provides:
...
## Choose the Responses State Model

Responses does not require Cloud Conversations. Preserve the application's existing Responses history pattern unless the user asks to change it:

- **Full `input` history:** keep the application storage and resend the intended history. Do not add a frontend-token route or Cloud conversation ownership checks.
- **`previous_response_id`:** keep the response-id chain and `store: true`. Persist and authorize the chain in the host application.
- **`conversation` plus `store: true`:** use the complete Cloud Conversations, frontend-token, and identity path below.

All three patterns can use the Responses generation endpoint and hosted tools. The client still consumes Responses events with the matching adapter, but only the named-conversation path uses `useOpenuiCloudStorage()` and sends just the latest turn against `conversation: threadId`.
...
For a custom component library, generate its serialized spec with `openui generate --spec`, pass that spec to `generateSystemPrompt({ cloud: true, library, ... })` in the generation route, and pass the matching runtime library to `AgentInterface.componentLibrary`. Follow [build-component-library.md](build-component-library.md); do not combine a custom client library with the built-in server prompt.
...
4. Confirm latest-turn-only forwarding, `conversation: threadId`, `stream: true`, and `store: true` remain intact.
