...
Use this runbook for application-code migration from OSS (open-source or self-hosted) OpenUI to the managed Cloud backend. Read [the shared Cloud integration guide](integration.md) and [Choose a chat generation API](chat/api-selection.md), then read the selected [Responses](chat/responses.md) or [Chat Completions](chat/chat-completions.md) runbook. Also read [Conversations](chat/conversations.md) when Cloud will own persistent threads.
...
1. Read [the Cloud integration guide](integration.md), [Responses](chat/responses.md), and [Conversations](chat/conversations.md), then add only the missing Cloud packages and server-only configuration.
